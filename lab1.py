    ...
    for item in items:
        ...
        try:
            if item['copy_history']:
                text = 'Эта запись содержит репост другой записи'
            else:
                text = ''
        except Exception:
            text = ''
        print('post_id: '+str(item['id'])+' date: '+date+' text: '+item['text']+' ' + text)
    return response
...
def editPost():
    ...
    if response == 1:
        ...
        print('Произошла ошибка при изменении сообщения сообщения')
    return response
...
if(os.path.isfile(filePath)):
    ...
    print('Введите логин и пароль для входа в учётную запись Вконтакте, а также своё имя')
    name = input("Имя: ")
    login = input("Логин: ")
